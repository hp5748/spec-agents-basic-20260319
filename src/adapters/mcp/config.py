...
class MCPConfig:
    ...
    register_level: Dict[str, MCPServerConfig] = field(default_factory=dict)  # register/mcp.json
    ...
    def get_all_servers(self) -> Dict[str, MCPServerConfig]:
        """获取所有服务器配置（优先级：yaml > register > project > user）"""
        all_servers = {}
        all_servers.update(self.user_level)
        all_servers.update(self.project_level)
        all_servers.update(self.register_level)
...
class MCPConfigLoader:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self._register_config_path = self._project_root / "register" / "mcp.json"
        self._user_config_path = Path.home() / ".claude.json"

    def load(self) -> MCPConfig:
        """加载所有配置（优先级：yaml > register > project > user）"""
        config = MCPConfig()
        ...
        # 2. 加载注册中心配置
        if self._register_config_path.exists():
            try:
                register_data = json.loads(self._register_config_path.read_text(encoding="utf-8"))
                if "mcpServers" in register_data:
                    for name, server_data in register_data["mcpServers"].items():
                        config.register_level[name] = self._parse_server_config(name, server_data)
                    logger.info(f"已加载注册中心 MCP 配置: {len(config.register_level)} 个服务器")
            except Exception as e:
                logger.warning(f"加载注册中心 MCP 配置失败: {e}")
